...
pub async fn do_wc(params: Value) -> DispatchResult {
    ...
    match (path, input) {
        ...
        (Some(_), Some(_)) => {
            Err("Provide either 'path', 'input', or 'paths', not multiple.".into())
        }
...
pub async fn do_lsof(params: Value) -> DispatchResult {
    ...
    let output = lsof::run_lsof(&arg_refs)
        .await
...
pub async fn do_kubectl_list(params: Value) -> DispatchResult {
    ...
    let result =
        kubectl::parse_list_response(&output, resource_type, ns).map_err(|e| e.to_string())?;
    serde_json::to_value(&result).map_err(|e| e.to_string())
}
...
pub async fn do_docker_list(params: Value) -> DispatchResult {
    ...
    let result = docker::list_containers(&client, all)
        .await
        .map_err(|e| e.to_string())?;
    serde_json::to_value(&result).map_err(|e| e.to_string())
}
...
pub async fn do_docker_inspect(params: Value) -> DispatchResult {
    ...
    let client = docker::connect().map_err(|e| e.to_string())?;
    let result = docker::inspect_container(&client, container)
        .await
        .map_err(|e| e.to_string())?;
    serde_json::to_value(&result).map_err(|e| e.to_string())
}
...
pub async fn do_docker_images(_params: Value) -> DispatchResult {
    let client = docker::connect().map_err(|e| e.to_string())?;
    let result = docker::list_images(&client)
        .await
        .map_err(|e| e.to_string())?;
    serde_json::to_value(&result).map_err(|e| e.to_string())
}
...
pub async fn do_ps(params: Value) -> DispatchResult {
    ...
    let pid_list: Option<Vec<u64>> = params
        .get("pid_list")
    ...
    let result = ps::list_processes(name_pattern, user, pid_list.as_deref(), max_results).await?;
    serde_json::to_value(&result).map_err(|e| e.to_string())
}
...
pub type DispatchFn =
    Arc<dyn Fn(Value) -> Pin<Box<dyn Future<Output = DispatchResult> + Send>> + Send + Sync>;

/// Build the dispatch table for all enabled tools.
pub fn build_dispatch_table(
    enabled: &Option<std::collections::HashSet<String>>,
) -> HashMap<String, DispatchFn> {
    let mut table: HashMap<String, DispatchFn> = HashMap::new();

    let is_enabled = |name: &str| -> bool { enabled.as_ref().is_none_or(|set| set.contains(name)) };
    ...
    macro_rules! register {
        ($name:expr, $func:ident) => {
            if is_enabled($name) {
                table.insert(
                    $name.to_string(),
                    Arc::new(|params| Box::pin($func(params))),
                );
            }
        };
    }
